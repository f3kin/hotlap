  ...
  it.each([false, true])(
    ...
    async (fail) => {
      const root = await NodeFSP.mkdtemp(
        NodePath.join(NodeOS.tmpdir(), "hotlap-install-progress-"),
      );
      const version = "1.2.3";
      const stem = `hotlap-${version}-linux-${HostProcessArchitecture.defaultValue()}`;
      ...
      const child = NodeChildProcess.spawn("script", ["-qec", `sh '${installer}'`, "/dev/null"], {
        env: {
          ...
          HOTLAP_HOME: NodePath.join(root, "home"),
          HOTLAP_INSTALL_BIN_DIR: NodePath.join(root, "bin"),
          HOTLAP_RELEASE_BASE_URL: `http://127.0.0.1:${address.port}`,
        },
      ...
      try {
        ...
        if (fail) {
          ...
          expect(output).not.toContain("Installed Hotlap");
          ...
          expect(output).toContain("Installed Hotlap 1.2.3");
          expect(
            await NodeFSP.readFile(NodePath.join(versions, version, ".install-complete"), "utf8"),
          ).toBe("1.2.3\n");
          expect(
            NodeChildProcess.execFileSync(NodePath.join(root, "bin/hotlap"), ["--version"], {
              encoding: "utf8",
            }).trim(),
          ).toBe("Hotlap 1.2.3");
