...
if ($interactive) {
  ...
  for ($i = 0; $i -lt $mark.Length; $i++) {
    ...
    $label = if ($i -eq 1) { "     ${bold}Hotlap$reset" } elseif ($i -eq 2) { "     ${muted}CLI installer$reset" } else { "" }
...
if ((Test-Path $marker -PathType Leaf) -and ((Get-Content $marker -Raw).Trim() -eq $version) -and (Test-Path (Join-Path $targetDir "t3.exe") -PathType Leaf)) {
  ...
  Write-Host "  hotlap $version is already installed at $targetDir`n"
} else {
...
[Console]::Error.WriteLine("  $muted$shim -> $(Join-Path $targetDir 't3.exe')$reset`n")
