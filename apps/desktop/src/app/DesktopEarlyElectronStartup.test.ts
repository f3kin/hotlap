...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("resolves the early linux Electron switches", () => {
    ...
    assert.deepEqual(options, {
      isDevelopment: true,
      linuxWmClass: "hotlap-dev",
      linuxDesktopEntryName: "ai.usefastlane.Hotlap.Development.desktop",
      passwordStore: "gnome-libsecret",
    });
  });

  it("keeps implicit development state under ~/.hotlap/dev when T3CODE_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.hotlap/dev/desktop-settings.json");
