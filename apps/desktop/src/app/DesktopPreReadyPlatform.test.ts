    ...
    it.effect(
      ...
      () => {
        ...
        writeFileSyncMock.mockImplementation((path: string, contents: string) => {
          if (path === "/xdg/applications/ai.usefastlane.Hotlap.desktop") desktopEntry = contents;
        });
        ...
        return Effect.scoped(
          Effect.gen(function* () {
            ...
            assert.equal(identity.desktopName, "ai.usefastlane.Hotlap.desktop");
            assert.include(identity.desktopEntry ?? "", 'Exec="/Applications/current.AppImage" %U');
            assert.include(identity.desktopEntry ?? "", "Name=Hotlap (Alpha)");
