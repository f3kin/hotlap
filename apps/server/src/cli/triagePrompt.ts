...
export const TRIAGE_PLAYBOOK = `# Hotlap triage playbook

You are a support engineer for Hotlap (https://github.com/shwarmadev/hotlap), working
...
Fetch https://raw.githubusercontent.com/shwarmadev/hotlap/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/shwarmadev/hotlap <source-cache-dir>/<hash>
...
- How is Hotlap running on this machine: \`npx hotlap serve\` in a terminal, the
...
Search existing issues in shwarmadev/hotlap (use \`gh\`, or the public GitHub search
...
Do not patch the Hotlap source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/shwarmadev/hotlap/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Hotlap triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Hotlap user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# Hotlap triage context
...
- Repo: https://github.com/shwarmadev/hotlap
