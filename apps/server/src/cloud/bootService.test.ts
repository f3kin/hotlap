...
const linuxRuntime = "/home/theo/.hotlap/runtime/versions/1.2.3/t3";
const linuxPlan = {
  program: [linuxRuntime, "__service-launcher"],
  baseDir: "/home/theo/.t3",
  logPath: "/home/theo/.t3/userdata/logs/boot-service.log",
  unitPath: "/home/theo/.config/systemd/user/hotlap.service",
};
...
it("runs the pinned runtime's own executable as the systemd launcher", () => {
  ...
  expect(unit).toContain(`Environment=HOTLAP_HOME=${linuxPlan.baseDir}`);
...
const macRuntime = "/Users/theo/.hotlap/runtime/versions/1.2.3/t3";
const macPlan = {
  program: [macRuntime, "__service-launcher"],
  baseDir: "/Users/theo/.t3",
  logPath: "/Users/theo/.t3/userdata/logs/boot-service.log",
  unitPath: "/Users/theo/Library/LaunchAgents/ai.usefastlane.code.service.plist",
};
...
it("runs the pinned runtime's own executable as the launch agent", () => {
  ...
  expect(plist).toContain(`<key>HOTLAP_HOME</key>\n    <string>${macPlan.baseDir}</string>`);
});
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const runtime = pinnedRuntimePaths(path, baseDir, "1.2.3", platform, nodePath);
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable hotlap.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart hotlap.service") control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop hotlap.service" ||
      ...
      return {
        stdout: input.args.includes("--version")
          ? // The runtime under test reports the version of the directory it
            ...
            `hotlap v${/versions\/([^/]+)\//.exec(command)?.[1] ?? "1.2.3"}\n`
  ...
  const makeService = (
    ...
    Effect.gen(function* () {
      ...
      const paths = pinnedRuntimePaths(path, serviceBaseDir, cliVersion, platform, nodePath);
      ...
      if (nodePath !== undefined)
        yield* fs.writeFileString(
          path.join(path.dirname(paths.entryPath), "service-launcher.mjs"),
          "export {};\n",
        );
      ...
      return yield* BootService.make({
        ...
        cliVersion,
        host: { execPath: nodePath ?? "/usr/bin/t3" },
      });
    }).pipe(
      ...
      Effect.provide(
        Layer.mergeAll(
          ...
          Layer.succeed(HostProcessIsExecutable, nodePath === undefined),
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  it.effect("keeps a Node-installed service on the npm runtime and its selected Node", () =>
    Effect.gen(function* () {
      ...
      const result = yield* service.install();
      const plist = yield* fs.readFileString(result.unitPath);
      expect(plist).toContain("<string>/custom/node</string>");
      expect(plist).toContain(
        `<string>${baseDir}/runtime/versions/1.2.3/node_modules/hotlap/dist/service-launcher.mjs</string>`,
      );
      expect(plist).not.toContain("<string>__service-launcher</string>");
    }),
  );
  it.effect(
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop hotlap.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now hotlap.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? ["systemctl --user stop hotlap.service", "systemctl --user restart hotlap.service"]
            : [
                "launchctl bootout --wait gui/501/ai.usefastlane.code.service",
  ...
  it.effect("install with start=false rewrites the files and marks a restart pending", () =>
    Effect.gen(function* () {
      ...
      // the reason is named so `hotlap service status` can point at restart.
  ...
  it.effect("restart stops and starts an installed service, and is a no-op otherwise", () =>
    Effect.gen(function* () {
      ...
      expect(
        ...
        "systemctl --user stop hotlap.service",
        "systemctl --user daemon-reload",
        "systemctl --user enable hotlap.service",
        "systemctl --user restart hotlap.service",
      ]);
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "ai.usefastlane.code.service.plist"),
        ),
      ).toBe(true);
      expect(yield* fs.readFileString(plan.unitPath)).toContain(
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/ai.usefastlane.code.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/ai.usefastlane.code.service",
        "launchctl enable gui/501/ai.usefastlane.code.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/ai.usefastlane.code.service";
