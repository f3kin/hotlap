...
<<<<<<< HEAD
if command -v hotlap >/dev/null 2>&1; then
  exec hotlap "$@"
fi
# npm extracts a package before it runs the native builds of its dependencies,
# so a failed build (hotlap depends on node-pty, which needs a C toolchain) leaves
# the npx cache without a hotlap executable. \`npx --yes\` then exits 0 without
...
require_installed_t3_cli() {
  if ! T3_CLI_PATH="$("$@" -- sh -c 'command -v hotlap')"; then
  ...
  printf 'Remote host installed %s but npm produced no hotlap executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@T3_PACKAGE_SPEC@@ >&2
  return 1
=======
...
<<<<<<< HEAD
printf 'Remote host is missing the hotlap CLI and could not install @@T3_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
=======
if [ -n "\${T3_LOCK:-}" ]; then
  rm -rf "$T3_LOCK"
...
<<<<<<< HEAD
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "hotlap@latest");
=======
>>>>>>> 6f00d3881a197dd33c2cb43c6a11a9e759e56089
