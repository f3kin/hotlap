...
describe("ssh tunnel scripts", () => {
  it("builds the remote t3 runner with npx and npm fallbacks", () => {
    ...
    assert.include(script, 'exec hotlap "$@"');
    assert.include(script, 'exec "$T3_CLI_PATH" "$@"');
    assert.include(script, "could not install 'hotlap@latest'");
    assert.include(script, "require_installed_t3_cli npx --yes --package 'hotlap@latest'");
    assert.include(script, "require_installed_t3_cli npm exec --yes --package 'hotlap@latest'");
    assert.include(script, "npm produced no hotlap executable");
